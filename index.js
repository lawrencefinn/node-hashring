...
HashRing.prototype.continuum = function generate() {
  ...
  servers.forEach(function each(server) {
    ...
    if (self.algorithm != "crc32"){
      ...
      for (var i =0; i < length * self.replicas; i++) {
        nodes.push(new Node(self.crc32HashKey(server.string + '-' + i, false, true), server.string));
      }
    }
  });
  ...
  if (this.algorithm == "crc32"){
    ...
    var i, step = 0xffffffff / self.MMC_CONSISTENT_BUCKETS;
    step = step >>> 0;
    for (i=0; i<self.MMC_CONSISTENT_BUCKETS; i++) {
      self.ring[i] = this.consistentFind(step * i, nodes);
    }
  }
...
HashRing.prototype.get = function get(key) {
  ...
  if (this.algorithm != "crc32") {
    node = this.ring[this.find(this.hashValue(key))];
  } else {
    node = this.ring[((this.crc32HashKey(key, false)%this.MMC_CONSISTENT_BUCKETS)+this.MMC_CONSISTENT_BUCKETS)%this.MMC_CONSISTENT_BUCKETS];
  }
 ...
 * @api private
 */
HashRing.prototype.hash = function hash(key, unsigned) {
  unsigned = typeof unsigned !== 'undefined' ? unsigned : false;
  if (this.algorithm == "crc32") {
    return this.crc32HashKey(key, unsigned);
  } 
...
HashRing.prototype.crc32HashKey = function(str, unsign){
  ...
  crc = crc ^ (-1);
  var ret = (crc < 0 && unsign ? crc += 4294967296 : crc);
  return ret;
};

HashRing.prototype.consistentFind = function(point, nodes) {
  var lo = 0, hi = nodes.length - 1, mid, self=this;
  while (1) {
    if (point <= nodes[lo].value || point > nodes[hi].value) {
      return nodes[lo];
    }
    mid = Math.floor(lo + (hi - lo) / 2);
    if (point <= nodes[mid].value && point > (mid ? nodes[mid-1].value : 0)) {
      return nodes[mid];
    }
    if (nodes[mid].value < point) {
      lo = mid + 1;
    }
    else {
      hi = mid - 1;
